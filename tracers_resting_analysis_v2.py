...
if activity_periods_analysis:
    for run_name in run_names:
        ...
        # PERFORM TEMPORAL ANAYSIS - FILL INACTIVE PERIODS <=N LONG
        tracers_matrix_filled = fill_consecutive_zeros(periods_matrix, 1)
        ...
        for i, key in enumerate(['Fill', 'Not detectable changes', 'Scour']):
            ...
            normalized_frequency_left = frequency_left / np.nansum(data)  # Normalize by total number of rows
            ...
            normalized_frequency_right = frequency_right / np.nansum(data)  # Normalize by total number of rows
