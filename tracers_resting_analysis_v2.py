...
if activity_periods__DoD_analysis:
    ...
    for run_name in run_names:
        ...
        # DoD calues for each category
        dod_scour = categories[categories < 0]
        dod_no_change = categories[categories == 0]
        dod_fill = categories[categories > 0]
        ...
        # You might want to aggregate the data for the scatter plot here
        dod_values = np.concatenate([dod_scour, dod_no_change, dod_fill])  # Concatenating all DoD-related values
        activity_durations = np.concatenate([scour_values, no_change_values, fill_values])  # Using the same resting_times as activity durations
        
        # Create the scatter plot
        fig, ax = plt.subplots(figsize=(8, 6))
        
        # Plot scatter
        ax.scatter(dod_values, activity_durations, c='green', alpha=0.6, edgecolors='w', s=80)
        
        # Set titles and labels
        ax.set_title(f"DoD vs Activity Duration - {run_name}", fontsize=14, weight="bold")
        ax.set_xlabel("DoD [mm]", fontsize=12)
        ax.set_ylabel("Resting Time", fontsize=12)
        
        # Add text or additional features
        plt.text(0.5, -0.1, f"Generated by: {os.path.basename(__file__)}", transform=plt.gca().transAxes, ha='center', va='center', fontsize=6)
        ...
        # Save the figure
        plt.savefig(os.path.join(plot_dir, run_name + '_DoD_vs_RestingTime_scatter.pdf'), dpi=800, format="pdf")
        
        # Display the plot
        plt.show()
        
        
        # Define the DoD bin edges
        DoD_bin_edges = [-60.0, -8.6, -5.8, -4.0, -2.6, -1.3, 1.3, 2.3, 3.7, 5.5, 8.3, 60.0]
        
        # Digitize the DoD values into bins
        dod_bins = np.digitize(dod_values, bins=DoD_bin_edges)
        
        # Prepare the data for the boxplot (group resting times by DoD bins)
        boxplot_data = [activity_durations[dod_bins == i] for i in range(1, len(DoD_bin_edges))]
        
        # Create the scatter and boxplot together
        fig, ax = plt.subplots(figsize=(10, 6))
        
        # Plot scatter
        # ax.scatter(dod_values, activity_durations, c='green', alpha=0.6, edgecolors='w', s=80, label="Scatter plot")
        
        # Boxplot for each DoD class
        ax.boxplot(boxplot_data, positions=np.arange(1, len(DoD_bin_edges)), widths=0.5, patch_artist=True,
        ...
        # Set DoD class labels on x-axis (the bin edges as labels)
        ax.set_xticks(np.arange(1, len(DoD_bin_edges)))
        ax.set_xticklabels([f"{DoD_bin_edges[i-1]} to {DoD_bin_edges[i]}" for i in range(1, len(DoD_bin_edges))], rotation=45)
        ...
        # Save the figure
        plt.savefig(os.path.join(plot_dir, run_name + '_DoD_vs_RestingTime_boxplot.pdf'), dpi=800, format="pdf")
