...
for run_name in run_names:
    ...
    for file in files:
        ...
        img_extract = ((band_green-band_red)>-22)*((band_red+band_green+band_blue)>350)*((band_red+band_green+band_blue)<700)*(band_green>100)*(band_red<190)*mask_filt_array # modified 2024/11/11
        
        # OVERLAP OLD EXTRACTIO AND NEW EXTRACTION
        tracers_extraction_img = img_extract+image_rmg
        tracers_extraction_img = np.where(tracers_extraction_img>0,1,0)
        
        tracers_extraction_img = fill_small_holes(tracers_extraction_img, max_hole_size=2, surround_threshold=5)
