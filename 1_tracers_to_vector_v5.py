...
for run_name in run_names:
    ...
    if not selected_row.empty:
        ...
        # DoD_name = selected_row['DoD timespan2'].values[0] # DoD name timespan 2 - combination 1
        # DoD_name = selected_row['DoD timespan2.1'].values[0] # DoD name timespan 2 - combination 2
    ...
    for file in files:
        ...
        if file != sorted(files,key = numericalSort)[-1]:
            ...
            tracers['x']    = tracers['Centroid'].x * scale_to_mm + x_0
            tracers['y']    = tracers['Centroid'].y * scale_to_mm + y_0
        ...
        elif file == sorted(files,key = numericalSort)[-1]:
            '''
            The last shot represent the photos taken with the fram moved downstream
            '''
            x_center2 = frame_position2 *1000 + 1100
            x_02 = x_center2 - 4288/2*scale_to_mm
            os.chdir(path_out)
            tracers.to_file(str(t) + "s_centroids.shp")
            tracers['x']    = tracers['Centroid'].x * scale_to_mm + x_0
            tracers['y']    = tracers['Centroid'].y * scale_to_mm + y_0
    ...
    feeding['x'] = feeding['Centroid'].x # Feeding position in photo CRS
    ...
    np.save(os.path.join(w_dir, 'output_data', 'output_images', run_name, 'tracers_reduced_'+ run_name +'.npy'),new_tracers)
    np.save(os.path.join(w_dir, 'output_data', 'output_images', run_name, 'tracers_appeared_reduced_'+ run_name +'.npy'),new_tracers_appeared)
    np.save(os.path.join(w_dir, 'output_data', 'output_images', run_name, 'tracers_disappeared_reduced_'+ run_name +'.npy'),new_tracers_disappeared)
    ...
    np.save(os.path.join(w_dir, 'output_data', 'output_images', run_name, 'tracers_appeared_reduced_stopped_'+ run_name +'.npy'),tracers_appeared_stopped)
    np.save(os.path.join(w_dir, 'output_data', 'output_images', run_name, 'tracers_disappeared_reduced_stopped_'+ run_name +'.npy'),tracers_disappeared_stopped)
