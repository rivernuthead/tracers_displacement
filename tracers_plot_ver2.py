...
for run in runs:
    ...
    plot_dir = os.path.join(w_dir, 'analysis_plot', run)
    plot_dir_appdisapp = os.path.join(w_dir, 'appdisapp', run)
   
    if os.path.exists(plot_dir):
        shutil.rmtree(plot_dir, ignore_errors=False, onerror=None)
        os.mkdir(plot_dir) 
    else: 
        os.mkdir(plot_dir)  
    if os.path.exists(plot_dir_appdisapp):
        shutil.rmtree(plot_dir_appdisapp, ignore_errors=False, onerror=None)
    ...
    tracers_reduced = np.load(path_in_tracers + '\\tracers_reduced_'+ run +'.npy',allow_pickle=True)
    tracers_appeared_reduced = np.load(path_in_tracers + '\\tracers_appeared_reduced_'+ run +'.npy',allow_pickle=True)
    tracers_disappeared_reduced = np.load(path_in_tracers + '\\tracers_disappeared_reduced_'+ run +'.npy',allow_pickle=True) 
    tracers_appeared_stopped = np.load(path_in_tracers + '\\tracers_appeared_reduced_stopped_'+ run +'.npy',allow_pickle=True)
    tracers_disappeared_stopped = np.load(path_in_tracers + '\\tracers_disappeared_reduced_stopped_'+ run +'.npy',allow_pickle=True) 
    ...
    for i in range(1,len(tracers_appeared_reduced)-1,1):
        dict_tracers = {'x':tracers_reduced[i,:,0],'y':tracers_reduced[i,:,1],'z':tracers_reduced[i,:,2],'z_DoD':tracers_reduced[i,:,3]}
        df_tracers = pd.DataFrame(dict_tracers)
        df_tracers = df_tracers.dropna()
        df_tracers = df_tracers.drop(df_tracers[df_tracers.x<100].index)
