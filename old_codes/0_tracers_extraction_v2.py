...
for run in runs:
    ...
    for file in sorted(files,key = numericalSort):
        ...
        img_extract = ((band_green-band_red)>-8)*((band_red+band_green+band_blue)>350)*((band_red+band_green+band_blue)<700) # modified 2024/10/20
    ...
    feeding.drop(feeding.index[feeding.area == max(feeding.area)], inplace = True) # Feeding contains a polygon that is theentire photo domain. Here this polygon is removed.
    feeding.drop(feeding.index[feeding.area < area_threshold], inplace = True)
    feeding.to_file(os.path.join(path_out, 'feeding_tracers.shp'))
